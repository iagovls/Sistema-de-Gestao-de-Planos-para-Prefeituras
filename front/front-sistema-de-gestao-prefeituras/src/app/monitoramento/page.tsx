...
import { useTodasPropostas } from "../hooks/usePropostas";
...
export default function Monitoramento() {
  ...
  const opcoesFiltros = useMemo((): OpcoesFiltros => {
    ...
    const prefeituras = [...new Set(propostas.map((p: Proposta) => p.prefeitura?.name).filter(Boolean))] as string[];
    const planos = [...new Set(propostas.map((p: Proposta) => p.plano?.nome).filter(Boolean))] as string[];
    const eixos = [...new Set(propostas.map((p: Proposta) => p.eixo?.nome).filter(Boolean))] as string[];
    const categorias = [...new Set(propostas.map((p: Proposta) => p.categoria?.nome).filter(Boolean))] as string[];
    const orgaosGestores = [...new Set(propostas.map((p: Proposta) => p.orgaoGestor?.nome).filter(Boolean))] as string[];
  ...
  const propostasFiltradas = useMemo(() => {
    ...
    return propostas.filter((proposta: Proposta) => {
      const matchPrefeitura = !filtros.prefeitura || proposta.prefeitura?.name === filtros.prefeitura;
      const matchPlano = !filtros.plano || proposta.plano?.nome === filtros.plano;
      const matchEixo = !filtros.eixo || proposta.eixo?.nome === filtros.eixo;
      const matchCategoria = !filtros.categoria || proposta.categoria?.nome === filtros.categoria;
      const matchOrgaoGestor = !filtros.orgaoGestor || proposta.orgaoGestor?.nome === filtros.orgaoGestor;
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {propostasPaginadas.map((proposta: Proposta) => (
                  <tr key={proposta.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{proposta.prefeitura?.name || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{proposta.plano?.nome || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{proposta.eixo?.nome || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{proposta.categoria?.nome || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{proposta.orgaoGestor?.nome || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
